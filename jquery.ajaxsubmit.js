(function($) {
     ...
     * @param {any} options 当设置为回调函数时，表单提交成功时将触发回调函数。
     *                        当设置为对象时，支持下面的属性：
    ...
    $.fn.ajaxSubmit = function(options) {
        $(this).submit(function(event) { //进行提交操作
            ...
            options.method = $this.attr('method') || options.type; //优先使用表单 method 属性
            options.data = {};

            $.each($this.serializeArray(), function() {
                options.data[this.name] = this.value; // 设置数据
            });
            ...
            if (options.setSelector && setLength) { //分组提交数据
                var i = t = 0,
                    _success = function(result, status, xhr) {
                        ...
                        if (successCallable) {
                            if (!options.once || i == setLength - 1)
                                success.call($this, t, status, xhr);
                        }
                ...
                $this.find(options.setSelector).each(function() {
                    ...
                    options.success = function(result, status, xhr) {
                        return _success(result, status, xhr);
                    };
